...
class CLIPVisionViPEmbeddings(nn.Module):
    def __init__(self, config: CLIPVisionConfig, additional_vision_config=None):
        ...
        self.keep_frame_cls = additional_vision_config.keep_frame_cls
        self.use_different_frame_cls = additional_vision_config.use_different_frame_cls
        self.wo_global_view_in_frame_attn = additional_vision_config.wo_global_view_in_frame_attn
        ...
        self.class_embedding = nn.Parameter(torch.randn(self.embed_dim))
        if self.keep_frame_cls:
            # place holder
            if self.use_different_frame_cls:
                self.cls_in_frames = nn.Parameter(torch.randn(self.temporal_size, self.embed_dim))
            else:
                self.cls_in_frames = nn.Parameter(torch.randn(self.embed_dim))
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        ...
        if self.keep_frame_cls:
            if self.use_different_frame_cls:
                # [1, T, 1, C]
                cls_in_frames = self.cls_in_frames.unsqueeze(0).unsqueeze(2)
                cls_in_frames = cls_in_frames.expand(B, -1, 1, -1)
            else:
                cls_in_frames = self.cls_in_frames.expand(B, T, 1, -1)
            if self.if_use_temporal_embed:
                cls_in_frames = cls_in_frames + time_embed.unsqueeze(2)
            cls_in_frames = cls_in_frames + self.position_embedding(self.position_ids[:, 0:1])

            # add cls within frame to patch_embeds
            patch_embeds = torch.cat([cls_in_frames, patch_embeds], dim=2)
        ...
        # TODO: hack code for wo_global_view_in_frame_attn
        if self.wo_global_view_in_frame_attn :
            return embeds, (M, N, L, 'wo_global_view_in_frame_attn')
...
class CLIPAttention(nn.Module):
    ...
    def forward2(self, hidden_states, inputs_size):
        ...
        inputs_size: (M, N, L)
        M is number of global tokens
        N is time length
        L is number of per frame tokens
        """
        bsz, _, C = hidden_states.size()
        if len(inputs_size) != 3 and inputs_size[-1] == 'wo_global_view_in_frame_attn':
            wo_global_view_in_frame_attn = True
            M, N, L, _ = inputs_size
        else:
            wo_global_view_in_frame_attn = False
            M, N, L = inputs_size
        ...
        if wo_global_view_in_frame_attn:
            # qkv: [B*num_heads, M+N*L, head_dim]
            # wo_global_view_in_frame_attn
            q = query_states[:, M:].reshape(-1, L, self.head_dim)  #[B*num_heads*N, L, head_dim]
            k = key_states[:, M:].reshape(-1, L, self.head_dim)  #[B*num_heads*N, L, head_dim]
            v = value_states[:, M:].reshape(-1, L, self.head_dim)  #[B*num_heads*N, L, head_dim]
            attn_weights = torch.bmm(q, k.transpose(1, 2)) #[B*num_heads*N, L, L]
            attn_weights = nn.functional.softmax(attn_weights, dim=-1)
            attn_probs = nn.functional.dropout(attn_weights, p=self.dropout, training=self.training)
            ...
            # qkv: [B*num_heads, M+N*L, head_dim]
            # in-frame attention:
            # q: within frame patches; k: within frame patches + global tokens; v: within frame patches + global tokens
            q = query_states[:, M:].reshape(-1, L, self.head_dim)  #[B*num_heads*N, L, head_dim]
            ...
            attn_weights = nn.functional.softmax(attn_weights, dim=-1)
            in_frame_attention_weights = attn_weights.reshape(bsz, self.num_heads, N, L, attn_weights.shape[-1])
            attn_probs = nn.functional.dropout(attn_weights, p=self.dropout, training=self.training)
            attn_output = torch.bmm(attn_probs, v)   # [B*num_heads*N, L, head_dim]
            attn_output = attn_output.view(bsz, self.num_heads, N, L, self.head_dim)
            attn_output = attn_output.permute(0, 2, 3, 1, 4)
        ...
        # q: global tokens; k: global tokens + all frame patches; v: global tokens + all frame patches
        q = query_states[:, :M]    # [B*num_heads, M, head_dim]
        k = key_states  # [B*num_heads, M+N*L, head_dim]
        v = value_states  # [B*num_heads, M+N*L, head_dim]
        attn_weights = torch.bmm(q, k.transpose(1, 2))
        attn_weights = nn.functional.softmax(attn_weights, dim=-1)
        divided_attention_weights = attn_weights.reshape(bsz, self.num_heads, M, attn_weights.shape[-1])
        attn_probs = nn.functional.dropout(attn_weights, p=self.dropout, training=self.training)
        ...
        return attn_output, divided_attention_weights
