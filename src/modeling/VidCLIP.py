...
class VidCLIP(nn.Module):
    ...
    def freeze_vision_encoder(self, freeze_vision_proj):
        freeze_list = [self.clipmodel.vision_model]
        # not_freeze_list = ['class_embedding', 'added_cls', 'cls_in_frames']
        if freeze_vision_proj:
            freeze_list.append(self.clipmodel.visual_projection)
        for m in freeze_list:
            m.eval()
