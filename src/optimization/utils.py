...
from src.optimization.lamb import Lamb
...
def build_optimizer_w_lr_mul(model_param_optimizer, learning_rate,
    ...
    if lr_mul_prefix == [""]:
        ...
        param_optimizer = [(n, p) for n, p in model_param_optimizer
                           if all((prefix not in n for prefix in lr_mul_prefix)) and p.requires_grad]

        param_top = []
        for prefix in lr_mul_prefix:
            param_top.extend([(n, p) for n, p in model_param_optimizer
                              if prefix in n and p.requires_grad])

        if hvd.rank() == 0:
            print('lr mul param names are {}'.format([n for n, p in param_top]))
...
def setup_e2e_optimizer(model, opts):
    ...
    all_param_optimizer = [(n, p) for n, p in list(model.named_parameters())]
