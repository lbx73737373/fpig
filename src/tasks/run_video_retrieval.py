...
def fk_validate(model, val_loaders, cfg):
    ...
    for loader_name, val_loader in val_loaders.items():
        ...
        generated_text_embeds = generated_text_embeds[:valid_len]

        sim_matrix = cal_cossim(text_embeds, vis_embeds)

        # cal metric of mse
        only_positive_pairs_mse_normed = cal_mse_in_text_to_pseudo(text_embeds, generated_text_embeds, normed=False)
...
def start_fk_training():
    ...
    loss_func = build_loss_func(cfg.loss_config)
    generation_func = build_generation_loss_func(cfg.loss_config)
    ...
    for step, batch in enumerate(InfiniteIterator(train_loader)):
        ...
        if cfg.loss_config.if_gather:
            ...
            if cfg.loss_config.loss_name in ["NCELearnableTempLoss", "NCELearnableTempDSLLoss"]:
                ...
                generation_loss = cfg.loss_config.genertion_loss_lambda * loss_func(generated_text_feats, text_feats,
                                                                                    temp=None)
        ...
        if step % 10 == 0:
            lr_ = optimizer.param_groups[0]['lr']
            LOGGER.info(
                'Step {}: loss {} | retrieval_loss {} | unscaled_generation_loss {} | lr {} | logit_scale {}'.format(
                    global_step, loss, retrieval_loss, generation_loss / cfg.loss_config.genertion_loss_lambda, lr_,
                    logit_scale_))
