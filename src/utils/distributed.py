...
def interleaved_allgather(tensor, dim=0):
    """
    Perform an allgather operation with interleaving on the specified dimension.
    step is fixed to 1.

    Args:
        tensor (torch.Tensor): Input tensor to gather and interleave.
        dim (int): The dimension along which to interleave the data.

    Returns:
        torch.Tensor: A tensor with data interleaved along the specified dimension.
    """
    # Step 1: Gather tensors from all processes
    gathered = hvd.allgather(tensor)

    # Step 2: Calculate interleaving indices
    total_size = gathered.size(dim)
    dim_size = tensor.size(dim)
    num_processes = hvd.size()

    # Create interleaving indices
    interleave_indices = []
    for i in range(dim_size):
        for j in range(num_processes):
            index = j * dim_size + i
            interleave_indices.append(index)

    interleave_indices = torch.tensor(interleave_indices, device=gathered.device)

    # Step 3: Interleave along the specified dimension
    interleaved = gathered.index_select(dim, interleave_indices)
...
if __name__=='__main__':
    hvd.init()
    rank = hvd.rank()
    features = torch.tensor([rank * 2, rank * 2 + 1, rank * 2 + 2], dtype=torch.float32)
    print(f"Rank {rank} has data {features}")
    result = interleaved_allgather(features, dim=0)

    print(f"Rank {rank} has gathered data {result}")